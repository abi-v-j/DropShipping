...
<br><br><br>
    ...
    <script>
        ...
        function addCart(id, max) {
            ...
            var newPrice = prompt("Enter a price");

            if (newPrice === null) {
                $("div.failure").fadeIn(300).delay(1500).fadeOut(400);

            } else {
                var priceAsNumber = parseFloat(newPrice);

                if (!isNaN(priceAsNumber)) {
                    if (priceAsNumber <= max) {
                        $.ajax({
                            url: "../Assets/AjaxPages/AjaxSellerAddCart.php?id=" + id +"&mprice="+priceAsNumber,
                            success: function (response) {
                        ...
                        alert("Invalid input. Please enter less than Max price "+ max);
                        addCart(id, max);
                    }
                } else {
        ...
        function productCheck() {
            ...
            $.ajax({
                url: "../Assets/AjaxPages/AjaxSellerSearchProduct.php?action=" + action + "&category=" + category + "&subcategory=" + subcategory + "&name=" + name,
                success: function (response) {
